...
const ListaIngredienteClient = () => {
  ...
  const fetchIngredientDetails = async (searchTerm) => {
    ...
    const details = await Promise.all(
      filtered.map((nome) => abc(nome.idIng))
    );

    setIngredientDetails(details);
  };

  const debouncedFetchIngredientDetails = useCallback(_.debounce(fetchIngredientDetails, 300), [nomes]);
          ...
          <tbody>
            {filteredNomes.length === ingredientDetails.length && // Ensure synchronization
              filteredNomes.map((nome, index) => (
                <tr key={nome.id}>
                  <td>{nome.idIng}</td>
                  <td>{ingredientDetails[index]?.name}</td> {/* Render ingredient name from state */}
                  <td>
                    <img src={ingredientDetails[index]?.image} alt="Ingredient" />
                  </td>
                  <td>{nome.unity}</td>
                  <td>{nome.quantity}</td>
