		...
		try {
			...
			if (user) {
				...
				ingredientIds.forEach((id) => {
					availabilityMap[id] = data.some(
						(product) => product.idIng === id
					);
				});
			...
			console.error(
				"Error checking ingredient availability:",
				error.message
			);
		}
	};
	...
	return (
		<div className="Recipe">
			...
			<div className="Detalhes">
				{user && (
					<FavoriteButton idRecipes={idrecipe} idUser={user.id} />
				)}
				<h2>{name}</h2>
				<div>
					<h3>Tags</h3>
				...
				<ol className="IngredientList">
					{ingsName.map((ing) => {
						const isIngredientAvailable =
							ingredientAvailability[ing.idingridients];
						return (
							<li
								key={ing.id}
								style={{
									backgroundColor: isIngredientAvailable
										? "green"
										: "red",
								}}
							>
